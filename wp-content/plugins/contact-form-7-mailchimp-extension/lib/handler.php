...
function errorHandler($errno, $errstr, $errfile, $errline, $errContext = []) {
    ...
    if (str_contains($errfile, 'contact-form-7-mailchimp-extension')) {
            ...
            if (isset($cf7_mch['CustomKey' . $i]) && isset($cf7_mch['CustomValue' . $i]) && strlen(trim($cf7_mch['CustomValue' . $i])) != 0) {
                ...
                if ($cf7_mch['CustomKey' . $i] == $birtday_field) {
                    ...
                    if ($date) {
                        ...
                        $err_trigger = trigger_error("Parameter Birthday was called incorrectly. Please check your Form-Tag names", E_USER_ERROR);
                        $chimp_php_log = new chimp_db_log('mce_php_issues', $logfileEnabled, 'api', $idform);
                        $chimp_php_log->chimp_log_insert_db(1, ' ===============  PARAMETER ERROR  =============== ' . "\n", "Parameter Birthday was called incorrectly. Please check your Form-Tag names");
                    }
                }
